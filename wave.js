...
function color(alpha){alpha*=waveGain;const p=blue?current.a:current.b,g=ctx.createLinearGradient(0,0,w,0);g.addColorStop(0,`rgba(${p[0].map(Math.round).join(',')},${alpha*.88*G})`);g.addColorStop(.38,`rgba(${p[1].map(Math.round).join(',')},${alpha*G})`);g.addColorStop(.67,`rgba(${p[2].map(Math.round).join(',')},${alpha*.98*G})`);g.addColorStop(1,`rgba(${p[3].map(Math.round).join(',')},${alpha*.88*G})`);return g}
const layers=22;
